...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QFrame, QApplication, QTabWidget, QFormLayout
)
...
class EditBehaviorDialog(QDialog):
    def __init__(self, behavior_name, subject, obj, is_subject_enum=False, is_object_enum=False,
        ...
        self.init_ui(behavior_name, subject, obj, is_subject_enum, is_object_enum, subject_enum_values, object_enum_values)

    def init_ui(self, behavior_name, subject, obj, is_subject_enum, is_object_enum, subject_enum_values, object_enum_values):
        ...
        # 创建行为主体和行为对象编辑器
        self.subject_editor = self.create_editor(is_subject_enum, subject_enum_values, subject, "请输入行为主体（如：司机）")
    ...
    def create_editor(self, is_enum, enum_values, initial_value, placeholder_text):
        ...
        if is_enum and enum_values:
            editor = QComboBox()
            editor.addItem("<空>")  # 添加空选项
            editor.addItems(enum_values)
            editor.setCurrentText(initial_value if initial_value in enum_values else "<空>")
        else:
            editor = QLineEdit()
    ...
    def validate_and_accept(self):
        ...
        校验输入是否合法，如果合法则关闭对话框。
        """
        subject = self.subject_editor.currentText() if isinstance(self.subject_editor, QComboBox) else self.subject_editor.text().strip()
        obj = self.object_editor.currentText() if isinstance(self.object_editor, QComboBox) else self.object_editor.text().strip()
...
class ElementSettingTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.static_data = {
            ...
            "应急行为要素": {
                ...
                "behavior": [
                    {"name": "快速响应", "subject": "应急指挥中心", "object": "各应急部门"},
                    {"name": "协调行动", "subject": "应急指挥官", "object": "救援队伍"}
                ]
            },
            "其他要素": {  # 第8个要素的静态数据
                "attributes": {
        ...
        # Initialize category_data with static_data
        self.category_data = {category: data.copy() for category, data in self.static_data.items()}
    ...
    def apply_three_line_table_style(self, table: QTableWidget):
        ...
        table.setStyleSheet("""
            ...
            /* 移除 QTableWidget::item:nth-child 样式，改用交替行颜色 */
            /* 设置选中行的样式 */
    ...
    def adjust_behavior_table_height(self):
        """根据内容调整行为表格的高度，使底部线条贴近内容"""
        total_row_height = sum(self.behavior_table.rowHeight(row) for row in range(self.behavior_table.rowCount()))
        header_height = self.behavior_table.horizontalHeader().height()
        # 计算表格总高度（行高 + 表头高 + 边距）
        table_height = total_row_height + header_height + 6  # 6 为微调值
        self.behavior_table.setFixedHeight(table_height)

    def force_refresh_behavior_table_headers(self):
        """确保表头样式刷新，防止表头下的线丢失"""
        self.behavior_table.horizontalHeader().setStyleSheet("""
            QHeaderView::section {
    ...
    def reset_behavior_model(self, placeholder_text="等待选择情景要素"):
        """辅助方法：重置行为模型区域到占位符状态"""
        # 隐藏表格
    ...
    def populate_behavior_model(self, behaviors):
        ...
        if behaviors:
            ...
            # 应用样式和调整高度
            self.apply_three_line_table_style(self.behavior_table)
            self.force_refresh_behavior_table_headers()
    ...
    def load_models_data(self, category):
        ...
        if current_attribute_labels:
            ...
            for label_text in current_attribute_labels:
                ...
                line_edit = QLineEdit()
                line_edit.setText(attributes.get(label_text, ""))
                # 根据字段类型设置验证器
                ...
                self.attribute_content_layout.addWidget(line_edit, row, col * 2 + 1)
                col += 1
                if col >= columns:
                    col = 0
                    row += 1
    ...
    def upload_model_file(self, category):
        ...
        file_dialog.setNameFilter("TXT文件(*.txt);;JSON 文件 (*.json);;模型文件 (*.model);;XML 文件 (*.xml);;所有文件 (*)")
        if file_dialog.exec():
            ...
            if selected_files:
                ...
                #         }
                #         self.checkboxes[category].label.set_uploaded(True)
                #         QMessageBox.information(self, "上传成功", f"已成功上传并加载要素模型文件:\n{file_path}")
                #         return True
                #     except Exception as e:
                #         QMessageBox.warning(self, "错误", f"解析 JSON 文件失败: {str(e)}")
                #         return False
                # elif file_path.endswith(('.model', '.xml')):
                #     QMessageBox.warning(self, "警告", "仅支持 JSON 文件格式的上传。")
                #     return False
                # else:
                #     QMessageBox.warning(self, "警告", "请选择有效的模型文件（.json）。")
                #     return False
        else:
            # 用户取消上传，不改变上传状态
            return False
    ...
    def open_behavior_editor(self, row, column):
        ...
        selected_row = row
        if selected_row < 0:
            CustomWarningDialog("警告","请选择一个行为进行编辑！").exec()
            return
        ...
        if not behavior_name_item:
            CustomWarningDialog("警告","行为名称为空，无法编辑。").exec()
            return
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            # 更新表格中的值
            if self.behavior_table.item(selected_row, 1):
            ...
            if self.behavior_table.item(selected_row, 2):
    ...
    def handle_save(self):
            ...
            if checkbox.checkbox.isChecked():
                ...
                if current_attribute_labels:
                    ...
                    for row in range(self.attribute_content_layout.rowCount()):
                        ...
                        line_edit = self.attribute_content_layout.itemAtPosition(row, 1).widget()
                        if isinstance(label_widget, QLabel) and isinstance(line_edit, QLineEdit):
        ...
        if not saved_categories:
            QMessageBox.information(self, "保存结果", "没有要保存的要素。")
            return
        ...
        QMessageBox.information(self, "保存成功", f"已保存以下要素数据:\n\n{save_message}")
    ...
    def populate_initial_display(self):
        ...
        # 1. 属性模型区域
        self.attribute_content_widget.hide()
        self.attribute_placeholder.setText("等待选择情景要素")
        self.attribute_placeholder.show()
    ...
    def handle_generate(self):
        # 模拟生成情景级孪生模型
        CustomInformationDialog("生成成功", "已成功生成情景级孪生模型。").exec()
...
if __name__ == "__main__":
    ...
    element_setting_tab = ElementSettingTab()
    layout.addWidget(element_setting_tab)
    window.setWindowTitle("情景要素设置")
